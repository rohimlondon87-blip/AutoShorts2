...
# --- KONFIGURASI DUAL TOKEN (PRIORITAS A) ---
# Branding: MICRO WILD Automated System
CREDENTIAL_SETS = [
    {
        'label': 'KUNCI UTAMA (A)',
    ...
    try:
        ...
        if creds.expired and creds.refresh_token:
            try:
                ...
                print(f"⚠️ {label}: Token Kedaluwarsa & Gagal Refresh.")
                return None
...
def get_video_rotation(path):
    """Mendeteksi rotasi video secara teknis (Paten Anti-Terbalik)."""
    try:
        cmd = [
...
def standardize_video(input_path, output_path):
    """Proses standarisasi: Tegak lurus, Landscape 16:9, dan Tanpa Suara."""
    rotation = get_video_rotation(input_path)
    filters = []
    
    # Perbaikan rotasi otomatis berdasarkan sensor HP
...
def main():
    ...
    # 1. Pilih Kunci Drive (A -> B)
    drive_service = None
    for cred in CREDENTIAL_SETS:
        drive_service = get_services(cred['token'], cred['secret'], cred['label'])
        if drive_service: 
            print(f"[*] Berhasil login menggunakan: {cred['label']}")
            break

    if not drive_service:
        print("⛔ SEMUA KUNCI GAGAL. Cek GitHub Secrets.")
        return

    try:
        # 2. Pindai Bahan di Drive
        v_res = drive_service.files().list(q=f"'{SOURCE_ID}' in parents and mimeType contains 'video' and trashed=false").execute()
        v_files = v_res.get('files', [])
        
        m_res = drive_service.files().list(q=f"'{MUSIC_ID}' in parents and mimeType contains 'audio' and trashed=false").execute()
        ...
        for i, m in enumerate(m_files[:15]): 
            mname = f"mus_{i}.mp3"
            if download_file(drive_service, m['id'], mname):
                m_list_content += f"file '{mname}'\n"
        with open("m_list.txt", "w") as f: f.write(m_list_content)

        # 4. Memulai Live Streaming dengan GOP Stabil (Anti-Buffering)
        rtmp = f"rtmp://a.rtmp.youtube.com/live2/{STREAM_KEY}"
        print(f"\n[🚀] SIARAN MICRO WILD DIMULAI (GOP Fixed 2s)")
        ...
        print("\n[*] Membersihkan file sementara di server MICRO WILD...")
        for f in os.listdir():
